...
cp_left <- ggplot(C_df_left_box, aes(x = x_pos, group = Group)) +
  ...
  geom_segment(
    aes(
      x = x_pos - 0.12,xend = x_pos + 0.12,y = median,  yend = median, colour = Group),
    linewidth = 1
  ) +
  geom_segment(
    aes(x = x_pos,xend = x_pos,y = lower, yend = q1, colour = Group),
    linewidth = 1
  ) +
  geom_segment(
    aes(x = x_pos,xend = x_pos,y = q3,yend = upper,colour = Group),
    linewidth = 1
  ) +
  geom_segment(
    aes(x = x_pos - 0.08,xend = x_pos + 0.08,y = lower,yend = lower,colour = Group),
    linewidth = 1
  ) +

  geom_segment(
    aes(x = x_pos - 0.08,xend = x_pos + 0.08,y = upper,yend = upper,colour = Group),
    linewidth = 1
  )+
