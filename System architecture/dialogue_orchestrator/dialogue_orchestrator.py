...
from prompts import DIALOGUE_AGENT_PROMPT, CONTINUE_CHAT_TIP
from utils import save_session, finish_session
...
class DialogueOrchestrator:
    ...
    def _handle_inquiry(self, user_input):
        ...
        tip = CONTINUE_CHAT_TIP.format(aspects=verdict[1])
    ...
    def run(self):
        ...
        while not self.is_terminal():
            # DELIVER_RISK / GUIDENCE present their output and then ask for
            # their own input internally; only INQUIRY consumes the loop-top
            # input. Skipping the prompt here avoids swallowing user input.
...
class Assistant:
    ...
    def __init__(self, user_info):
        self.dialogue_agent_system_prompt = DIALOGUE_AGENT_PROMPT
